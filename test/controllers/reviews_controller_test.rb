...
class ReviewsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should return NOT FOUND if product does not exist' do
    assert_raises(ActionController::RoutingError) do
      get "#{root_path}/some-product-slug-that-does-not-exist", as: :html
    end
  end
  ...
  test 'create review should broadcast new review to live_reviews stream' do
    ...
    recent_broadcasts = broadcasts(LiveReviewsChannel.broadcasting_for(@product))
    recently_broadcast_review = ActiveSupport::JSON.decode(recent_broadcasts[0])['reviews'][0]
